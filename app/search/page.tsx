  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      {query ? (
        <>
          {tests.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-sm text-gray-400 mb-6">Patikrinkite rašybą arba bandykite kitą pavadinimą</p>
        ...
        <div className="text-center py-16">
          <p className="text-4xl mb-4">🔬</p>
          <p className="text-gray-500 mb-8 text-sm">Įveskite tyrimo pavadinimą paieškos laukelyje</p>
          {categories.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Arba naršykite kategorijas</p>
              <div className="flex flex-wrap justify-center gap-2">
                {categories.slice(0, 6).map((cat) => (
                  <Link
                    key={cat.slug}
                    href={`/category/${cat.slug}`}
                    className="px-4 py-2 rounded-lg border border-gray-200 bg-white text-sm text-gray-600 hover:border-blue-300 hover:text-blue-600 transition-colors"
                  >
